...
TACTICAL_ACTIVE_REMAINING = 80
TACTICAL_HEAD_DISTANCE = 6
...
SAFE_FOOD_IMMEDIATE_BONUS = 75_000
SAFE_FOOD_PATH_BONUS = 45_000
...
def _edge_distance(grid, pos):
    """Distancia a un borde real del tablero (las barras laterales ya son '#')."""
    if not pos or not grid:
        return INF
    r, c = pos
    max_r = len(grid) - 1
    # Columnas 0 y last son paredes '#'; la zona jugable empieza en 1.
    max_c_play = len(grid[0]) - 2
    return min(r, max_r - r, max(0, c - 1), max(0, max_c_play - c))
...
def _tactical_solver_should_run(grid, my_body, en_body, remaining):
    ...
    # V6.1 conservador: no pagar 5-7 ms por posiciones abiertas.
    #  - movilidad crítica (0/1 salida): siempre;
    #  - endgame con cuerpos grandes: sí;
    #  - contacto cercano con alguna serpiente comprimida: sí;
    #  - presión de borde cercana: sí.
    return (
        len(my_moves) <= 1
        or len(en_moves) <= 1
        or (remaining <= 50 and combined_len >= 24)
        or (head_dist <= 4 and combined_len >= 14 and (len(my_moves) <= 2 or len(en_moves) <= 2))
        or (edge <= 1 and head_dist <= 6 and combined_len >= 16
            and (len(my_moves) <= 2 or len(en_moves) <= 2))
    )
...
def _force_target_death(grid, my_body, en_body, food_list, my_is_a,
    ...
    target_is_my=True  -> ¿el rival puede FORZAR nuestra muerte?
    target_is_my=False -> ¿podemos FORZAR la muerte rival?

    Devuelve True/False si queda demostrado dentro del horizonte y None si el
    presupuesto se agota. El jugador que intenta matar usa EXISTE; la víctima
    usa PARA TODO (elige cualquier escape que rompa la fuerza).
    """
    if stats['nodes'] >= TACTICAL_SOLVER_NODE_BUDGET or time.monotonic() >= deadline:
        stats['aborted'] = True
        return None

    moving_body = my_body if turn_is_my else en_body
    other_body = en_body if turn_is_my else my_body
    moving_head = moving_body[0] if moving_body else None
    moves = get_legal_moves_raw(grid, moving_head, moving_body, other_body) if moving_head else []

    if not moves:
        moving_side_is_target = (turn_is_my == target_is_my)
        return moving_side_is_target
    if plies_left <= 0:
        return False
    ...
    # El jugador que NO es la víctima intenta forzar la muerte.
    forcing_turn = (turn_is_my != target_is_my)
    saw_unknown = False

    # Orden barato: el forzador prueba primero jugadas que reducen movilidad
    # de la víctima; la víctima prueba primero jugadas con más aire.
    ordered = []
    moving_is_a = my_is_a if turn_is_my else (not my_is_a)
    for name, pos in moves:
        u = make_move(grid, moving_body, pos, moving_is_a, 0, food_list)
        try:
            ...
            unmake_move(grid, moving_body, u[0], u[1], u[2], u[3], moving_is_a, pos, food_list, u[5])
    ...
    if saw_unknown:
        return None
    result = False if forcing_turn else True
    memo[key] = result
    return result


def compute_tactical_trap_scores(grid, my_body, en_body, food_list, my_is_a,
                                 candidates, remaining, global_deadline):
    """Prueba raíz selectiva para pérdidas/ganancias forzadas por encierro.

    Fase 1 (prioritaria): analiza TODAS las jugadas para no entrar en muerte forzada.
    Fase 2 (oportunista): busca mate rival solo en posiciones de baja movilidad y
    con un presupuesto mucho menor. Así la ofensiva nunca roba el tiempo de defensa.
    """
    ...
    start = time.monotonic()
    local_deadline = min(global_deadline, start + TACTICAL_SOLVER_TIME_BUDGET)
    stats['enabled'] = True
    loss_penalties = zero.copy()
    win_bonuses = zero.copy()
    original = (tuple(tuple(r) for r in grid), tuple(my_body), tuple(en_body), tuple(food_list))

    # 1) DEFENSA: todos los candidatos antes de gastar un nodo en ataque.
    defense_memo = {}
    for move_name, npos in candidates:
        if time.monotonic() >= local_deadline or stats['nodes'] >= TACTICAL_SOLVER_NODE_BUDGET:
            stats['aborted'] = True
            break
        u = make_move(grid, my_body, npos, my_is_a, 0, food_list)
        ...
        try:
            enemy_forces_us = _force_target_death(
                ...
                local_deadline, stats, defense_memo
            )
            ...
            unmake_move(grid, my_body, u[0], u[1], u[2], u[3], my_is_a, npos, food_list, u[5])

    # 2) OFENSIVA: solo candidatos no fatales donde el rival ya está comprimido.
    # Se usa un subpresupuesto; si no se demuestra mate, no se inventa bonus.
    offense_deadline = min(local_deadline, time.monotonic() + TACTICAL_OFFENSE_TIME_BUDGET)
    offense_memo = {}
    if time.monotonic() < offense_deadline:
        for move_name, npos in candidates:
            if loss_penalties.get(move_name, 0) > 0:
                continue
            if time.monotonic() >= offense_deadline or stats['nodes'] >= TACTICAL_SOLVER_NODE_BUDGET:
                break
            u = make_move(grid, my_body, npos, my_is_a, 0, food_list)
            before_nodes = stats['nodes']
            try:
                ...
                head_dist = (
                    abs(my_body[0][0] - en_body[0][0]) + abs(my_body[0][1] - en_body[0][1])
                    if en_body else INF
                )
                if len(enemy_moves) <= 2 or head_dist <= 4:
                    we_force_enemy = _force_target_death(
                        grid, my_body, en_body, food_list, my_is_a,
                        False, False, TACTICAL_OFFENSE_MAX_PLIES,
                        offense_deadline, stats, offense_memo
                    )
    ...
    now = (tuple(tuple(r) for r in grid), tuple(my_body), tuple(en_body), tuple(food_list))
    if now != original:
...
def _clear_straight_food_distance(grid, start, food):
    ...
    sr, sc = start; fr, fc = food
    if sr != fr and sc != fc:
        return INF
    dr = 0 if sr == fr else (1 if fr > sr else -1)
    dc = 0 if sc == fc else (1 if fc > sc else -1)
    dist = abs(fr - sr) + abs(fc - sc)
    if dist <= 0:
        return 0
    r, c = sr, sc
    for _ in range(dist):
        r += dr; c += dc
        if (r, c) == food:
            return dist if grid[r][c] == '*' else INF
        if not is_pos_inside(grid, (r, c)) or grid[r][c] != ' ':
            return INF
    return INF
...
def compute_safe_food_opportunity_adjustments(grid, my_body, en_body, food_list,
    ...
    out = {name: 0 for name, _ in candidates}
    details = {}
    if not food_list or not my_body:
        return out, details

    # En LOCK dejamos que Apple Freeze gobierne; en cashout final sí cobramos.
    mode = strategy.get('mode')
    rem = strategy.get('remaining', 300)
    lock_freeze_window = (mode == 'LOCK' and rem > APPLE_BANK_CASHOUT_REMAINING)

    for name, npos in candidates:
        if tactical_losses.get(name, 0) > 0 or fe_penalties.get(name, 0) > 0:
            continue
        if tactical_wins.get(name, 0) > 0:
            continue

        immediate = npos in food_list
        if immediate:
            if lock_freeze_window:
                continue
            out[name] = SAFE_FOOD_IMMEDIATE_BONUS
            details[name] = {'food': npos, 'distance_after_root': 0, 'immediate': True}
            continue
        ...
        try:
            my_head = my_body[0]
            en_head = en_body[0] if en_body else None
            # Rival mueve primero después de nuestra raíz: exigimos ventaja ESTRICTA.
            en_dists = bfs_distances(grid, en_head, en_body[-1] if en_body else None) if en_head else {}
            best = None
            for f in food_list:
                d = _clear_straight_food_distance(grid, my_head, f)
                if d == INF or d > SAFE_FOOD_MAX_STEPS - 1:
                    continue
            ...
            if best is not None:
                d, ed, f = best
                bonus = SAFE_FOOD_PATH_BONUS + max(0, SAFE_FOOD_MAX_STEPS - d) * SAFE_FOOD_STEP_BONUS
                out[name] = bonus
                details[name] = {'food': f, 'distance_after_root': d, 'enemy_distance': ed, 'immediate': False}
        finally:
            unmake_move(grid, my_body, u[0], u[1], u[2], u[3], my_is_a, npos, food_list, u[5])
...
def choose_direction(game_data, use_fe=USE_FUTURE_ESCAPE_DEFAULT, use_ac=USE_ANTI_CYCLE_DEFAULT,
    ...
    tactical_loss_penalties, tactical_win_bonuses, tactical_stats = compute_tactical_trap_scores(
        grid, my_body, en_body, food_list, my_is_a, candidates, remaining, deadline
    )

    safe_food_adjustments, safe_food_details = compute_safe_food_opportunity_adjustments(
        grid, my_body, en_body, food_list, my_is_a, candidates, strategy,
        fe_penalties, tactical_loss_penalties, tactical_win_bonuses
    )
    ...
    LAST_STRATEGY_STATS.update({
        ...
        "safe_food_adjustments": dict(safe_food_adjustments),
        "safe_food_details": safe_food_details,
    })
    ...
    while time.monotonic() < deadline and depth <= 12:
        ...
        for move_name, npos in candidates:
            ...
            # V6.1: resultados TÁCTICOS demostrados dominan heurísticas suaves.
            score -= tactical_loss_penalties.get(move_name, 0)
            score += tactical_win_bonuses.get(move_name, 0)
            score += safe_food_adjustments.get(move_name, 0)
            if use_ac:
...
def run_tests():
    ...
    # V6.1 Tactical Trap Solver ofensivo: ocupar la última salida rival debe ser mate.
    kg = [list("#####") for _ in range(5)]
    kg[1] = list("#  B#")
    kg[2] = list("#aA##")
    kg[3] = list("#   #")
    kmy = deque([(2,2),(2,1)])
    ken = deque([(1,3)])
    kcands = get_legal_moves_raw(kg, kmy[0], kmy, ken)
    kloss, kwin, kstats = compute_tactical_trap_scores(
        kg, kmy, ken, [], True, kcands, 20, time.monotonic()+1.0
    )
    assert kwin.get('UP', 0) == TACTICAL_FORCED_WIN_BONUS, (kcands, kloss, kwin, kstats)
    assert kloss.get('UP', 0) == 0
    print('[OK] Tactical Trap Solver detecta mate ofensivo inmediato')

    # Safe Food: una comida recta cercana y ganable recibe bonus; una fatal no.
    sg2 = [list("#######"), list("# A * #"), list("# a   #"), list("# a B #"), list("#######")]
    smy = deque([(1,2),(2,2),(3,2)])
    sen = deque([(3,4)])
    sfoods = [(1,4)]
    scands = get_legal_moves_raw(sg2, smy[0], smy, sen)
    sstate = compute_strategy_state(0,0,200); sstate['remaining']=200
    sfe = {n:0 for n,_ in scands}; stl={n:0 for n,_ in scands}; stw={n:0 for n,_ in scands}
    sadj, sdet = compute_safe_food_opportunity_adjustments(
        sg2, smy, sen, sfoods, True, scands, sstate, sfe, stl, stw
    )
    assert sadj.get('RIGHT',0) > 0, (scands, sadj, sdet)
    stl['RIGHT'] = TACTICAL_FORCED_LOSS_PENALTY
    sadj2, _ = compute_safe_food_opportunity_adjustments(
        sg2, smy, sen, sfoods, True, scands, sstate, sfe, stl, stw
    )
    assert sadj2.get('RIGHT',0) == 0
    print('[OK] Safe Food Opportunity premia comida recta solo si no es fatal')
