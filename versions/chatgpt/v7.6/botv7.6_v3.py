...
def compute_apple_freeze_penalties(grid, my_body, en_body, food_list, candidates,
    ...
    """Penaliza cobrar ahora una comida que conviene mantener congelada."""
    ...
    if _apple_freeze_inactive(strategy, remaining, len(food_list)):
        return penalties, stats
    food_set = set(food_list)
    safe_non_food = _safe_non_food_names(candidates, food_set, fe_penalties, forced_trap_penalties)
    if not safe_non_food:
        return penalties, stats
    my_topo = _topology_for_body_or_empty(grid, my_body)
    en_topo = _topology_for_body_or_empty(grid, en_body)
    bankable, enemy_urgent = _classify_freeze_food(food_list, my_topo, en_topo)
    ...
    if not _freeze_strength_ok(len(bankable), safe_concede, enemy_urgent):
        return penalties, stats
    _apply_freeze_penalties(penalties, candidates, food_set, bankable, en_topo, safe_concede)
...
def _freeze_disabled_stats(penalties, grid):
    reason = "numbered_food_v3" if numbered_food_mode(grid) else "disabled_or_deadline"
    return {
...
def _apple_freeze_layer(ctx, use_apple_freeze, fe_penalties, forced_penalties, search_mode, deadline):
    effective = use_apple_freeze and not numbered_food_mode(ctx["grid"])
...
# =====================================================================
# ENTRYPOINT: runtime separado del motor estratégico.
# =====================================================================
def main():
