...
# ==========================================
# 2. EL CEREBRO DEL BOT (PATHFINDING REAL)
# ==========================================
...
def get_real_distance_to_food(grid, start_pos, food_positions):
    ...
    queue = [(start_pos, 0)]  # (posición, distancia)
    visited = set([start_pos])
    rows = len(grid)
    cols = len(grid[0]) if rows > 0 else 0
    ...
    return float('inf') # Si no hay camino posible a la comida
...
def choose_direction(game_data):
    ...
    # 1. Filtro estricto: Movimientos que no causen muerte inmediata
    safe_moves = []
    ...
    # 2. Radar de supervivencia: Buscar los caminos que no nos encierren
    move_spaces = {}
    for move_name in safe_moves:
        move_spaces[move_name] = count_free_space(grid, directions[move_name])
    
    max_space = max(move_spaces.values())
    
    # Nos quedamos SOLO con los movimientos que nos dan el máximo espacio para vivir
    best_surviving_moves = [m for m in safe_moves if move_spaces[m] == max_space]

    if not food_list:
        return best_surviving_moves[0]

    # 3. Pathfinding real a la comida (Solo usando los caminos seguros)
    best_move = best_surviving_moves[0]
    min_dist = float("inf")

    for move_name in best_surviving_moves:
        next_pos = directions[move_name]
        dist = get_real_distance_to_food(grid, next_pos, food_list)
