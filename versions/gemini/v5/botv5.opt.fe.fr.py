...
def evaluate_state_v5(grid, my_body, en_body, my_score, en_score, remaining, food_list):
    ...
    if remaining < 30:
        if score_diff > 0:
            return base_score + (my_topo.space * 50) + (my_topo.exits * 250) + choke_bonus
        return base_score + (food_tempo_score * 4) + (food_race_bonus * 2) + (my_territory * 15) + choke_bonus
    elif is_losing:
        return base_score + (food_tempo_score * 3) + int(food_race_bonus * 1.5) + (my_territory * 25) + mobility_score + choke_bonus
    return base_score + (food_tempo_score * 2) + food_race_bonus + (my_territory * 18) + mobility_score + choke_bonus
...
def alpha_beta(grid, depth, alpha, beta, is_my_turn, my_body, en_body,
    ...
    state_hash = hash((
        ...
        tuple(sorted(food_list)), remaining, my_is_a
    ))
...
def choose_direction(game_data, use_fe=USE_FUTURE_ESCAPE_DEFAULT):
    global TT, SEARCH_ABORTED, LAST_FE_STATS
    TT.clear()
    SEARCH_ABORTED = False
...
# =====================================================================
def run_tests():
    print("=== V5-OPT + FUTURE ESCAPE V1: TESTS ===")
    ...
    tt_key = hash((tuple(tt_my), tuple(tt_en), False, tuple(sorted(tt_food)), 100, True))
...
def _validate_and_get_token():
    if len(sys.argv) < 2:
        print("Uso: python SNAKE_V5_OPT_FE_FR_V1.py <TU_TOKEN_JWT>")
        sys.exit(1)
    return sys.argv[1]
