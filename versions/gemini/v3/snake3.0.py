#bot de snake con voronoi y combate, con un motor gráfico mejorado (pygame) y mejorado
import asyncio
import json
import sys
import websockets
...
def draw_board(game_data):
    """Dibuja el tablero, la cuadrícula, el marcador de puntos y los turnos."""
    global screen, font
    
    board_raw = game_data.get("board", "")
    
    # 🛠️ FIX COLUMNA EXTRA: strip() a cada línea para matar espacios y \r fantasmas
    grid = [list(line.strip()) for line in board_raw.strip().split("\n") if line.strip()]
    rows = len(grid)
    ...
    if screen is None:
        ...
        pygame.display.set_caption("🐍 Torneo de Snake - lautaromolina")
    ...
    # --- INICIO DEL CÓDIGO NUEVO DEL RELOJ ---
    turn_data = game_data.get("turn_data", game_data)
    remaining_moves = game_data.get("remaining_moves", 300)
    turn = 300 - remaining_moves
    ...
    txt_turn = f"⏱️ Turno: {turn}/300"
    # --- FIN DEL CÓDIGO NUEVO DEL RELOJ ---

    # Renderizar los textos en pantalla
    render_p1 = font.render(txt_p1, True, (50, 255, 50))
    render_p2 = font.render(txt_p2, True, (50, 150, 255))
    render_turn = font.render(txt_turn, True, (255, 255, 255))
    
    screen.blit(render_p1, (10, rows * CELL_SIZE + 15))
    screen.blit(render_p2, (10, rows * CELL_SIZE + 45))
    
    # Posicionar el contador de turnos a la derecha del HUD
    turn_x_pos = cols * CELL_SIZE - 150
    screen.blit(render_turn, (max(turn_x_pos, 200), rows * CELL_SIZE + 30))
...
def count_free_space(grid, start_pos, max_spaces=250):
    queue = [start_pos]
...
def evaluate_move(grid, next_pos, food_list, enemy_head, my_length, enemy_length, turn):
    max_search_space = 250
    free_space = count_free_space(grid, next_pos, max_spaces=max_search_space)
    dist_to_food = get_real_distance_to_food(grid, next_pos, food_list)
    
    SCORE = 0
    
    # 1. 🛡️ FILTRO ESTRICTO DE SUPERVIVENCIA (La cura al encierro)
    if free_space <= my_length + 1:
        SCORE -= 100000  # Evita los espacios más chicos que nuestro propio cuerpo
        
    # Si la ruta no lleva al tablero abierto (free_space < max_search_space), es un callejón.
    if free_space < max_search_space:
        SCORE -= (max_search_space - free_space) * 50  # Fuerte penalización por entrar a bolsillos cerrados
    else:
        SCORE += 20000  # Prioridad absoluta: mantenernos en zonas abiertas
...
# Variables globales para la memoria de la cola
historial_mi_cabeza = []
mi_longitud_anterior = 0

def choose_direction(game_data):
    global historial_mi_cabeza, mi_longitud_anterior
    ...
    # ==========================================
    # 🧠 SISTEMA DE MEMORIA: LIBERACIÓN DE COLA
    # ==========================================
    if head != (0, 0): # Asegurarnos de que encontramos nuestra cabeza
        historial_mi_cabeza.append(head)
        
    # Mantener el tamaño del historial igual a nuestra longitud actual
    while len(historial_mi_cabeza) > my_length:
        historial_mi_cabeza.pop(0)
        
    # Si tenemos el historial completo y NO acabamos de comer (es decir, no crecimos)
    # sabemos exactamente dónde está la punta de nuestra cola y que se va a mover.
    if len(historial_mi_cabeza) == my_length and my_length == mi_longitud_anterior:
        cola_r, cola_c = historial_mi_cabeza[0]
        # TRUCO MÁGICO: Borramos nuestra cola de la matriz virtual. 
        # Así, TODOS los algoritmos (Voronoi, BFS, is_safe) la verán como espacio libre.
        grid[cola_r][cola_c] = ' '

    # Guardamos la longitud actual para el próximo turno
    mi_longitud_anterior = my_length
    # ==========================================
